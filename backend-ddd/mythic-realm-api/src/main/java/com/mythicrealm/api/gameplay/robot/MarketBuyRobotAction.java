...
public class MarketBuyRobotAction implements RobotDecisionAction {
    ...
    public RobotActionScore score(RobotDecisionContext context) {
        if (!canRun(context)) {
            return RobotActionScore.zero("商会暂无可承担的高品质装备");
        }
        double value = 28;
        ...
        if (context.personalityContains("商会") || context.personalityContains("低价") || context.personalityContains("装备")) {
            value += 12;
        }
        if (context.isCurrentKind("market_buy")) {
            value -= 16;
        }
        String reason = "商会有 " + context.marketOpportunities() + " 件可承担的稀有以上装备，买装可能比硬刷更快提升";
